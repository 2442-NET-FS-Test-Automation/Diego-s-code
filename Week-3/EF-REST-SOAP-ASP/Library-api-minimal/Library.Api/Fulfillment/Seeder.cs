...
//In "production" our orders would come from users. There API's run locally
//So we could either - create a post for a single order an run a shell script or something 
//or we could create a seeding endpoint from here to generate some orders for us.

public interface ISeeder
{
    IReadOnlyList<int> SeedOrders(int n, bool excpedited);
...
public class Seeder : ISeeder
{
    ...
    public IReadOnlyList<int> SeedOrders(int n, bool expedited)
    {
        ...
        //Create a dictionary based on our product table (the Id's in the db) and the skuss
        var pid = db.Products.ToDictionary(p => p.Sku, p => p.Id); //SKU keu, productId value
        ...
        for(int i = 0; i < n; i++)
        {
            var order = new Order
            {
                CustomerId = Random.Shared.Next(1, 3), //Random number - bounden.
    ...
    public IReadOnlyList<int> ResetAndCreateOrders(int n)
    {
        ...
        db.SaveChanges(); //Save changes after reset
