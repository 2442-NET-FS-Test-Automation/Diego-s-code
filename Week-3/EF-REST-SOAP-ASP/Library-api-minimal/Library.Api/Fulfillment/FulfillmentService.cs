...
public class FulfillmentService : IFulfillmentService
{
    ...
    //Method tp resolve skus to productId using that dictionary
    public int ResolveProductId(string sku)
    {
       try{ return _skuToProductId[sku]; }
        catch (KeyNotFoundException) {throw new UnknowSkuExpetion(sku); }
    }
    ...
    public async Task<BurstResult> FulfillBurstAsync (IEnumerable<int> orderIds, CancellationToken ct)
    {
        ...
        var task = orderIds.Select(id => FulfillOneAsync(id, ct)); //each call will get its own DbContext
        ...
        return new BurstResult(
            ...
            Fulfilled: result.Count(r => r == FulfillmentResult.Fulfilled),
            Backordered: result.Count(r => r == FulfillmentResult.Backordered)
        );
