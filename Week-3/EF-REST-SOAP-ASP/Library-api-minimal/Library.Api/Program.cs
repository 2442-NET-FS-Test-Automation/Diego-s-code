...
var conn_string = "Server=localhost,1433;Database=LibraryMinimalDb;User Id=sa;Password=Deniro_007;TrustServerCertificate=true";
...
//Burst endpoint
//For going creating a record - we will take these from a query string
//IHostAplicationLifeTime - this Lets us see events related to the app Lifetime
...
app.MapGet("/Orders/burst", (int n, bool expedited, ISeeder seeder,
    ...
    {
        var ids = seeder.SeedOrders(n, expedited); //Callimg the seed orders method with the stuff from front end
        var appStoping = Lifetime.ApplicationStopping; //Gives us a cancellation token that is called when app goes shutdown

        _ = Task.Run( async () => //Assingning the task result to a discardd runs this as a background task
        {
            try
            {
                using var scope = scopes.CreateAsyncScope(); //ask for fresh scope
                var service = scope.ServiceProvider.GetRequiredService<IFulfillmentService>(); //Grab a fulfillment service
                await service.FulfillBurstAsync(ids, appStoping); //use it to call
            }
            catch(Exception ex)
            {
                //This task is fore and forget because we aren't waitting or storing its result
                //any exception would be "swallowed" i.e. they would die with the task in the background
...
{
    var rows = db.Inventory.Include( i => i.Product).ToList(); //Grab Inventory 
...
app.MapPost("/bechnmark", async (int n, IFulfillmentService fs, ISeeder seeder, CancellationToken ct)=>
{

    //Lets see how squential vs parallel runs compare - with mixed orders
...
app.MapGet("/reports/by-completion", (LibraryDbContext db) =>
{
    return db.Orders //Look inside orders table
        ...
        .Select(o => new {o.Id, o.Priority, o.CompletedUtc}) //use info from those orders to make some returne objects
        .ToList();
});
...
// Binary search on the stored result
app.MapGet("/Reports/rank-of/{units:int}", (int units, LibraryDbContext db) =>
{
   ...
   var unitsDesc = db.FulfillmentEvents
    ...
    //1000, 400, 330, 34
    //Our BinarySearch needs a comparer - for something like a int or a char this is easy
        ...
        IDbContextFactory<LibraryDbContext> dbf, CancellationToken ct) =>
{
    ...
    catch(UnknowSkuExpetion ex)
    {
        Log.Warning("Rejected order: uknow SKU: {Sku}", ex.Sku);
